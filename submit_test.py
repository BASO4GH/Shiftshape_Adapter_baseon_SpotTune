...
def prepare_data_loaders(dataset_names, data_dir, imdb_dir, shuffle_train=True, index=None):
    ...
    picklePath=data_dir+'decathlon_mean_std.pickle'
    with open(picklePath, 'rb') as handle:
        dict_mean_std = pickle.load(handle, encoding='bytes')
    dict_mean_std_bytes_type=dict_mean_std
    dict_mean_std={ key.decode(): val for key, val in dict_mean_std.items() }
    ...
    for i in range(len(dataset_names)):
        ...
        # dict_mean_std=dict_mean_std_bytes_type
...
val_loaders = prepare_data_loaders(list(datasets.keys()), args.datadir, args.imdbdir, True)
...
f =  "./adp_results_2020_0112.json"
with open(f, "w") as fh:
    json.dump(results, fh, default=convert)    
print(f+' generated')
